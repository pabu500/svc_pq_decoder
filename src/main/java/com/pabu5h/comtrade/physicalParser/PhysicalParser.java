...
//            }
    ...
    public RecordHeader readRecordHeader(InputStream stream) throws IOException {
        // Track the position manually
        long position = 0; // Start position at 0 (or wherever your stream starts)

        // Read the first 64 bytes (record header)
        ...
        // Create a new RecordHeader object
        RecordHeader recordHeader = new RecordHeader();
        // Parse fields
        recordHeader.setPosition(position); // Use the updated position here
        recordHeader.setRecordSignature(new UUID(buffer.getLong(), buffer.getLong())); //16 byte
        recordHeader.setRecordTypeTag(new UUID(buffer.getLong(), buffer.getLong())); //16 byte
        recordHeader.setHeaderSize(buffer.getInt()); //4 byte
        recordHeader.setBodySize(buffer.getInt()); //4 byte
        recordHeader.setNextRecordPosition(buffer.getInt()); //4 byte
        recordHeader.setChecksum(buffer.getInt() & 0xFFFFFFFFL); //4 byte
        recordHeader.setReserved(new byte[16]);
        position += bytesRead; // Update the position after reading
        return recordHeader;
    }
